...
export class ChatDroneWidget extends CompositePanelWidgetBase {

    static LABEL = "Chat: Drone";

    static DEFAULT_WIDTH = 4;
    static DEFAULT_HEIGHT = 12;

    constructor(panel) {
        super(panel, 'chat-drone');

        // Get custom config if available
        let default_chat_topic = this.client.getConfigParam('chat_drone.topic') || '/drone/chat';

        // Chat history storage
        this.messages = [];
        this.maxMessages = 100;
        this.storageKey = 'chat_drone_messages';
        this.promptsStorageKey = 'chat_drone_active_prompts';
        ...
        this.prompts_el = $('<div class="prompt-list drone-prompts"></div>');
        ...
        if (this.messages.length === 0) {
            this.addMessage('system', 'Connected to Drone chat interface');
        }
    }
    ...
    onChatMessage(topic, msg) {
        ...
        this.addMessage('drone', msg.data);
    }
    ...
    renderDetection(data) {
        ...
        const message = { sender: 'drone', text: `[Detection] ${infoText} (${cameraShort})`, timestamp };
    ...
    addMessage(sender, text) {
        ...
        const senderClass = sender === 'user' ? 'user-message' :
                           sender === 'drone' ? 'bot-message' : 'system-message';
    ...
    renderPrompts() {
        ...
        for (const prompt of this.activePrompts) {
            let chip = $('<span class="prompt-chip drone-prompt-chip"></span>');
    ...
    restoreMessagesToUI() {
        for (const message of this.messages) {
            const senderClass = message.sender === 'user' ? 'user-message' :
                               message.sender === 'drone' ? 'bot-message' : 'system-message';
    ...
    onClose() {
        console.log('Closing Chat: Drone widget');
        super.onClose();
    }
}
